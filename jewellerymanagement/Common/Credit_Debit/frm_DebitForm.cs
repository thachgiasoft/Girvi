...
{
    public partial class frm_DebitForm : Form
    {
        public frm_DebitForm()
        {
            InitializeComponent();
        }
        ...
        private void clearAllFields()
        {
            ...
            DataTable dtDebit = _objCredit.getallAccountLedgerDetailsDebit();
            for (int i = 0; i < dtDebit.Rows.Count; i++)
            {
                dgv_DebitData.Rows.Add(dtDebit.Rows[i]["CreditDebitmasterid"].ToString(), dtDebit.Rows[i]["date"].ToString(), dtDebit.Rows[i]["debit"].ToString(), dtDebit.Rows[i]["FromAccount"].ToString(), dtDebit.Rows[i]["chequeno"].ToString(), dtDebit.Rows[i]["chequedate"].ToString());
            }
            DataTable dt = _objCredit.getallAccountLedgerDetails();
