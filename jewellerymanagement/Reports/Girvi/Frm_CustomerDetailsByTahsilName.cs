...
{
    public partial class Frm_CustomerDetailsByTahsilName : Form
    {
        ...
        public Frm_CustomerDetailsByTahsilName()
        {
            InitializeComponent();
        }
        ...
        private void Frm_CustomerDetailsByTahsilName_Load(object sender, EventArgs e)
        {
            SetFont();

            cmb_TahsilName.DataSource = _objGirvi.retrunCustomerAddress2();
            cmb_TahsilName.SelectedIndex = -1;
            dtp_StartDate.Focus();
        }
        public void ShivajiFont()
        {
            if (rbt_OtherTrue.Checked == true)
            {
                ...
                cmb_TahsilName.Font = new Font(pfc.Families[0], 16, FontStyle.Regular);
        ...
        private void fillGirviData()
        {
            try
            {
                ...
                DataTable dtCustomer = _objGirviCommon.getKhatawaniByTahsil(cmb_TahsilName.Text);
                ...
                for (int i = 0; i < dtCustomer.Rows.Count; i++)
                {
                    ...
                    {
                        if (dtGirvi.Rows[j]["Amount"].ToString() == "Release")
                        {
                            string desc_item = "";
                            double itemamount = 0;
        ...
        private void bttn_view_Click(object sender, EventArgs e)
        {
            ...
            {
                dataGridView_GirviDetails.Rows.Clear();
                DataTable dtCustomer = _objGirviCommon.getKhatawaniByAddress2(cmb_TahsilName.Text);
                DataTable dtGirvi = null;
                int SrNo = 1;
                for (int i = 0; i < dtCustomer.Rows.Count; i++)
                {
        ...
        private void cmb_VillageName_TextChanged(object sender, EventArgs e)
        {
            ...
            //    }
            //    cmb_TahsilName.DataSource = _objGirvi.retrunCustomerAddress();
